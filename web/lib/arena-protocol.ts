...
export function getArenaProtocol() {
  return {
    ...
    raritySystem: {
      status: "exact onchain snapshot live, gameplay modifiers pending",
      active: false,
      activeFeatures: ["rarity-rank-api", "rarity-tier-api"],
      pendingFeatures: ["free-rare-modifiers", "mogs-burn-modifiers", "modifier-resolution"],
      ...
      tiers: {
        common: "no free modifier; planned one fixed $MOGS burn modifier when enabled",
        uncommon: "no free modifier; planned one fixed $MOGS burn modifier when enabled",
        rare: "planned one free minor modifier per match",
        epic: "planned one free modifier per match",
        legendary: "planned capped free modifiers per match",
      },
